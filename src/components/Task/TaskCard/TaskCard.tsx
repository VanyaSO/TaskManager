...
import { getStatusClass } from "../../../shared/utils/task.ts";
...
import {
  deleteTaskAction,
  updateTaskAction,
} from "../../../shared/store/taskSlice.ts";
...
export function TaskCard({ task, messageApi, onUpdate }: TaskCardProps) {
  ...
  return (
    <Card
      ...
      style={{ backgroundColor: task.options?.backgroundColor }}
      styles={{
        body: bodyStyles,
    ...
    >
      ...
      {task.finalizedAt && (
        <div className={styles.dateLine}>
          {statusIcons[task.status]} {task.finalizedAt}
        </div>
      )}
